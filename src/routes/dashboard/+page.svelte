<script lang="ts">
	...
	import { IconCoin, IconPlus, IconChartBar, IconAlertTriangle, IconCheck, IconRotate, IconBooks } from '@tabler/icons-svelte';
	...
	{#if exams.length === 0}
		<section class="page-stack">
			<div class="card-accent violet">
				...
				<h1 class="mt-1.5 text-2xl font-black leading-tight text-white sm:text-3xl">Welcome, {displayName}!</h1>
				<p class="mt-1.5 text-sm leading-6" style="color: var(--text-muted); max-width: 400px;">
					You haven't created any exams yet. Let's change that.
				</p>
			</div>
			...
			<div class="card" style="text-align: center; padding-block: 2.5rem;">
				<div class="mx-auto flex h-16 w-16 items-center justify-center rounded-2xl" style="background: var(--violet-soft);">
					<IconBooks size={32} stroke-width={1.5} style="color: var(--violet);" />
				</div>
				<h2 class="mt-4 text-xl font-black text-white">No exams yet</h2>
				<p class="mx-auto mt-2 max-w-xs text-sm leading-6" style="color: var(--text-muted);">
					Paste your study notes and let AI generate a custom exam. Start practicing in seconds.
				</p>
				<div class="mt-6 flex flex-col items-center gap-2 sm:flex-row sm:justify-center">
					<Button href="/generate" class="px-6 py-3">
						<IconPlus size={18} stroke-width={2.5} /> Create your first exam
					</Button>
				</div>
			</div>
