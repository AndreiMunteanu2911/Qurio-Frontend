<script lang="ts">
	...
	const SKIN_PREVIEW: Record<string, { label: string; css: Record<string, string> }> = {
		skin_minimal: { label: 'Minimal', css: { border: '2px solid rgb(255 255 255 / 0.16)', background: '#120922', boxShadow: '0 6px 0 #070216' } },
		skin_gradient: { label: 'Gradient', css: { border: '2px solid #6849ff', background: 'linear-gradient(135deg, #170b35, #211151 52%, #112d42)', boxShadow: '0 7px 0 #070216' } },
		skin_neon: { label: 'Neon', css: { border: '2px solid #69eff7', background: 'linear-gradient(145deg, #100624, #092f3d)', boxShadow: '0 7px 0 #070216, 0 0 20px rgb(105 239 247 / 0.28)' } },
		...
		skin_classic: { label: 'Classic', css: { border: '2px solid rgb(255 255 255 / 0.1)', background: '#130a2b', boxShadow: '0 7px 0 #070216' } },
		skin_terminal: { label: 'Terminal', css: { background: 'linear-gradient(145deg, #03120d, #062719)', border: '2px solid #56ff92', boxShadow: '0 7px 0 #070216' } },
		skin_hologram: { label: 'Hologram', css: { background: 'linear-gradient(135deg, #160831, #25205a 35%, #07334a 70%, #3a2d0a)', border: '2px solid #7dd3fc', boxShadow: '0 7px 0 #070216, 0 0 20px rgb(125 211 252 / 0.22)' } },
		skin_paper: { label: 'Paper', css: { background: 'linear-gradient(145deg, #2b1c10, #170d07)', border: '2px solid #c8ad7f', boxShadow: '0 7px 0 #070216' } },
		skin_obsidian: { label: 'Obsidian', css: { background: 'linear-gradient(145deg, #080506, #160709 55%, #050101)', border: '2px solid #ff3045', boxShadow: '0 7px 0 #070216' } },
	};
	...
	<section class="page-stack">
		...
		<div class="flex flex-wrap gap-1.5">
			{#each SHOP_CATEGORIES as cat}
				<button
					class={['tag pressable theme-card-button', category === cat.id ? 'tag-cyan is-active' : 'tag-surface']}
		...
		<div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
			{#each filtered as item}
				...
				{@const equipped = slot ? equippedInSlot(slot) === item.id : false}
				<button
					class="theme-card-button flex cursor-pointer flex-col rounded-[var(--radius-sm)] p-[0.9rem] text-left transition-transform active:translate-y-px {equipped ? 'is-equipped' : ''}"
					onclick={() => (selectedItem = item)}
				>
					<div class="flex items-start justify-between">
						<div class="flex items-center gap-2">
							<div class="theme-icon-chip flex h-8 w-8 items-center justify-center rounded-lg">
